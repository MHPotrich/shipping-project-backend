...
async function getAddressDataBase(): Promise<void> {
    ...
    addressDataBase = await global.mongoClient.db(global.dataBaseName).collection("address");
}
...
export default class AddressDAO {
    ...
    static async getAddress(id: number): Promise<Address|null> {
        ...
        try {
            ...
            const address: Address = new Address(addressMongo.country, addressMongo.state, addressMongo.city, addressMongo.address1, addressMongo.address2, addressMongo.zipCode, id);
    ...
    static async getAllAddresses(): Promise<Address[]|null> {
        ...
        try {
            ...
            cursor.toArray().forEach(item => users.push(new Address(item.country, item.state, item.city, item.address1, item.address2, item.zipCode, item.id)));
    ...
    static async updateAddress(id: number, newContent: object): Promise<any> {
        ...
        try {
            ...
            return await addressDataBase.updateOne({ _id: address["_id"] }, { $set: newContent });
        } catch(error){
            console.error(error);
            return null;
        }
