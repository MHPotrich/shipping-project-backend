...
async function getShippingDataBase(): Promise<void> {
	...
	shippingDataBase = await global.mongoClient
...
export default class ShippingDAO {
	...
	static async getShipping(shippingCode: number): Promise<Shipping | null> {
		...
		try {
			const shippingMongo = await shippingDataBase.findOne({
				shippingCode: shippingCode,
			});
			const destination: Address = await AddressDAO.getAddress(
				shippingMongo.destination
			);
			const sendedBy: User = await UserDAO.getUser(
				shippingMongo.sendedBy
			);
			const sendedFor: User = await UserDAO.getUser(
				shippingMongo.sendedFor
			);
	...
	static async updateShipping(shippingCode: number, newContent: object) {
		...
		try {
			const shipping = await shippingDataBase.findOne({
				shippingCode: shippingCode,
			});

			return await shippingDataBase.updateOne(
				{ _id: shipping["_id"] },
				{ $set: newContent }
			);
		} catch (error) {
